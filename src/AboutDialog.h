...
public:
    static void Show(bool* p_open) {
        ...
        if (ImGui::Begin("О программе", p_open, 
            ...
            ImGui::TextColored(ImVec4(0.2f, 0.6f, 1.0f, 1.0f), ICON_FA_CIRCLE_INFO " Financial Audit");
            ...
            ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), "Назначение:");
            ImGui::Text(
                "Программа предназначена для автоматизированного аудита "
                "финансовых операций, анализа договоров, отслеживания платежей "
                "и выявления подозрительных операций в финансовых документах."
            );
            ImGui::PopTextWrapPos();
            ImGui::Spacing();

            // Применяемые технологии
            ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), "Используемые технологии:");
            ...
            ImGui::Separator();
            ImGui::Spacing();
            ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "Разработчик:");
            ImGui::SameLine();
            ...
            // Кнопка закрытия
            ImGui::SetCursorPosX((ImGui::GetWindowContentRegionMax().x - ImGui::CalcTextSize("Закрыть").x) * 0.5f);
